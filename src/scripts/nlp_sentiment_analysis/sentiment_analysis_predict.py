...
class SentimentAnalysis:
    ...
    def _preprocess_excel_data(self, rel_file_path: str) -> pd.DataFrame:
        ...
        assert len(
            excel_workbook) != 0, "Error: Expected a non-zero count of sheets to be found in the Excel Workbook"
        ...
        LOGGER.debug(
            df_customer_survey_responses_filtered["answerFreeTextValues"].notna().value_counts())
    ...
    def predict_sentiment_analysis(self) -> Tuple[pd.DataFrame, Dict[str, float]]:
        assert (
            self._df_customer_survey_responses["sentiment"] == ""
        ).all(), 'Error: Expected all "sentiment" values to be empty before attempting to predict sentiment analysis'
        ...
        with open(
            ...
            sentiment_pred_results_pandas_series: pd.Series = pd.Series(
                map(lambda pred_entry: label_encoder_mapping.get(int(cast(str, pred_entry))), y_pred), name="sentiment"
            )
            self._df_customer_survey_responses = pd.concat(
                [self._df_customer_survey_responses,
                    sentiment_pred_results_pandas_series], axis=1
            )
            ...
            label_counts: Dict[int, float] = {label: np.sum(
                y_pred == label) for label in np.unique(y_pred)}
            LOGGER.debug(f"Label Counts: {label_counts}")

            for label, count in label_counts.items():
                sentiment_label = label_encoder_mapping.get(
                    label)  # Get the label from the mapping
