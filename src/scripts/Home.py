...
st.set_page_config(page_title="MDAHub", page_icon="🎖️", layout="wide")
...
with selecter:
    ...
    if isinstance(daterange_chosen, tuple) and len(daterange_chosen) == 2:
        ...
        st.write(f"You have selected the range: **{start_date}** to **{end_date}**")
        ...
        if is_start_first_day and is_end_last_day:
            if end_date >= start_date:
                st.success("✅ The selected range represents one or more complete months.")
                
            else:
                st.error("❌ Error: The end date cannot be before the start date.")

        else:
            st.warning("⚠️ The selected range does **not** represent full month(s).")

    elif daterange_chosen:
        st.warning("Please select a valid date *range* (both start and end dates).")
    ...
    if is_valid_full_month and category_chosen:
        ...
        st.button("Confirm", key="confirm_button", type="primary", disabled=True)
        st.caption("Please select a valid date range and at least one category to enable this button.")
    ...
    if confirm_button := st.session_state.get("confirm_button"):
        if confirm_button:
             with SessionLocal() as session:
                ...
                df = query_facts(session=session, metric_ids=metric_ids, group_names=["all"], period_levels=[2], date_from=daterange_chosen[0], date_to=daterange_chosen[1])
                ...
                for key, item in results.get("result", {}).items():
                    ...
                    if period_key is not None and value is not None:
                        ...
                        markdown_output.write(f"**Value:** {formatted_value}\n\n")
                    else:
                ...
                if len(markdown_string.strip()) > 0: # Check if markdown is not just whitespace
                    ...
                    pdf_file_path_on_server = generate_pdf(_markdown=markdown_string)
                    ...
                    if pdf_file_path_on_server: # Check if PDF generation succeeded
                        try:
                            ...
                            st.download_button(
                                ...
                                data=pdf_bytes,  # <-- Pass the actual PDF content (bytes)
                            ...
                            st.error(f"Error: Could not find the generated PDF file.")
                            LOGGER.error(f"FileNotFoundError trying to read {pdf_file_path_on_server} for download.")
                        except Exception as e:
                            st.error(f"Error preparing PDF for download: {e}")
                            LOGGER.error(f"Error reading PDF file {pdf_file_path_on_server} for download: {e}", exc_info=True)
                    else:
                ...
                st.markdown("### Report Preview (Markdown)") # Add a header for clarity
