import sqlite3
import pandas as pd
from src.scripts.data_warehouse.util import aggregate_metric_by_group_hierachy, aggregate_metric_by_time_period, get_metric_1_lowest_level, insert_facts_from_df
...
df = aggregate_metric_by_time_period(_metric_id=1, lowest_level=df, _method='sum')
LOGGER.info(df.dtypes)
LOGGER.info(df.shape)
...
LOGGER.info(insert_facts_from_df(df_facts=aggregate_metric_by_group_hierachy(_metric_id=1, _method='sum')))
