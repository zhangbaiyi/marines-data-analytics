...
if __name__ == "__main__":
    ...
    LOGGER.info(convert_jargons(session = session, df = query_facts(session=session, metric_id=1, group_names=['1100'], period_levels=[2], exact_date=date(2024,12,1))))
    LOGGER.info(convert_jargons(session = session, df = query_facts(session=session, metric_id=1, group_names=['1100'], period_levels=[1], date_from=date(2025,1,1), date_to=date(2025,1,3))))
    LOGGER.info(convert_jargons(session = session, df = query_facts(session=session, metric_id=1, group_names=['1100'], period_levels=[3], exact_date=date(2024,10,1))))
    LOGGER.info(convert_jargons(session = session, df = query_facts(session=session, metric_ids=[1], group_names=['1100', '5206', '2301'], period_levels=[4], exact_date=date(2025,1,1))))
