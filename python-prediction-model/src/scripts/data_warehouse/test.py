...
if __name__ == "__main__":
    ...
    LOGGER.info("""What is the total sales of 
                    HHM MCX MAIN STORE (Henderson Hall Main Store) in 
                        December 2024? """)
    LOGGER.info(query_facts(session=session, metric_id=1, group_names=[
    ...
    LOGGER.info(query_facts(session=session, metric_id=1, group_names=[
                '1100'], period_levels=[2], date_from=date(2025, 1, 1), date_to=date(2025, 1, 3)))
    LOGGER.info("""What is the total sales of 
                                        HHM MCX MAIN STORE (Henderson Hall Main Store) 
                          compared with CLM MCX MAIN STORE (Camp Lejelle Main Store)
                                                 On January 1st 2025? """)
    LOGGER.info(query_facts(session=session, metric_id=1, group_names=[
                '1100', '5100'], period_levels=[2], exact_date=date(2025, 1, 1)))
    LOGGER.info("""What is the total sales of 
                        HHM MCX MAIN STORE (Henderson Hall Main Store) 
                                    On 4Q24 and 1Q25? """)
    LOGGER.info(query_facts(session=session, metric_id=1, group_names=[
                '1100'], period_levels=[3], date_from=date(2024, 10, 1), date_to=date(2025, 1, 1)))
    LOGGER.info("""What is the total sales 
                           and [Another metric] of 
                                    HHM MCX MAIN STORE (Henderson Hall Main Store) 
                                        On Jan 1st 2025? """)
    LOGGER.info(query_facts(session=session, metric_ids=[1, 2], group_names=[
                '1100'], period_levels=[1], exact_date=date(2025, 1, 1)))

    LOGGER.info(getMetricByID(session=session, metric_id=1))
    LOGGER.info(getSiteByID(session=session, site_id='1100'))
    LOGGER.info(convert_jargons(session=session, df=query_facts(session=session,
                metric_id=1, group_names=['1100'], period_levels=[2], exact_date=date(2024, 12, 1))))
    LOGGER.info(convert_jargons(session=session, df=query_facts(session=session, metric_id=1, group_names=[
                '1100'], period_levels=[1], date_from=date(2025, 1, 1), date_to=date(2025, 1, 3))))
    LOGGER.info(convert_jargons(session=session, df=query_facts(session=session,
                metric_id=1, group_names=['1100'], period_levels=[3], exact_date=date(2024, 10, 1))))
    LOGGER.info(convert_jargons(session=session, df=query_facts(session=session, metric_ids=[
                1], group_names=['1100', '5206', '2301'], period_levels=[4], exact_date=date(2025, 1, 1))))
